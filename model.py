...
class CTRhythm(nn.Module):
    def __init__(self, init_channels = 1, d_model = 32, nhead=8, num_layers = 6, dim_feedforward = 256, num_classes = 4, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return x
class CNN(nn.Module):
    def __init__(self, channel_in = 1, d_model = 32,  num_classes = 4, dropout=0.3):
        super().__init__()
        self.CNNModule= CNNModule(channel_in = channel_in, downsample_depth = 5, main_depth = 1 ,dropout = dropout)
        self.classification = nn.Linear(d_model, num_classes)    
    def forward(self, x):
        x = self.CNNModule(x) # [batch_size, d_model, seq_len]
        x = torch.mean(x, dim=2)  # [batch_size, d_model]
