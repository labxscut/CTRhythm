...
#CPSC/Chapman
def evaluate_model_all(model, data_loader, criterion, device='cpu'):
    model.to(device)  
    model.eval()
    y_true = []
    y_pred = []
    ...
    # F1All
    f1_all = sum(f1_scores[:]) / (len(f1_scores))
    ...
    return conf_matrix, f1_scores,  f1_all, accuracy, average_loss
#CINC 2017
...
def train_model_all(train_loader, test_loader, model, criterion, optimizer, num_epochs, device='cpu',evaluate=True):
    model.to(device)  
    for epoch in range(num_epochs):
        model.train()
        total_loss = 0.0
        ...
        if evaluate and epoch % 5 == 4:
            ...
            print(
                f"Epoch [{epoch + 1}/{num_epochs}] - Train Loss: {average_train_loss:.3f} - Test Loss: {test_loss:.3f} - Accuracy: {accuracy:.3f} - F1 All: {f1_all:.3f} - F1 Scores: {', '.join([f'{f1_score:.3f}' for f1_score in f1_scores])}")
