...
if __name__ == '__main__':
    ...
    data_dir = "/work1/lzy/data/CPSC2018/"
    dataset = CPSCDataset(data_dir,sampling_rate = 150, max_length=15,lead = 1)
    ...
    for num, method in enumerate(method_list):
        ...
        for fold in range(5):
            ...
            print(f"Accuracy: {fold_metrics[fold]['accuracy']:.3f} - F1 All: {2*fold_metrics[fold]['f1_all']:.3f} - F1 Scores: {', '.join([f'{fold_metrics[fold]['f1_scores'][i]:.3f}' for i in range(2) ])}")
    ...
    all_results_path = filedir + '/result/val_cpsc_results.csv'
